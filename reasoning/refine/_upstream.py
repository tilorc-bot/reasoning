...
def ask(proposition: SymPyExpr,
        assumptions: SymPyExpr | bool = True) -> bool | None:
    """Answer a predicate query with the independent engine.

    SymPy's ``ask`` first consults direct predicate handlers, old
    assumptions, and the linear arithmetic solver; this wrapper answers from
    :func:`reasoning.satask.satask` alone.  Global assumptions are not read,
    matching how the validation suite binds ``ask`` to the engine.
    """
    return satask(proposition, assumptions)
...
def refine_exp(expr: Basic, assumptions: Boolean | bool) -> Basic | None:
    """
...
def refine_atan2(expr: Basic, assumptions: Boolean | bool) -> Basic:
    """
...
def refine_re(expr: Basic, assumptions: Boolean | bool) -> Basic:
    """
...
def refine_im(expr: Basic, assumptions: Boolean | bool) -> Basic:
    """
...
def _refine_reim(expr: Basic, assumptions: Boolean | bool) -> Basic | None:
...
def refine_matrixelement(expr: Basic, assumptions: Boolean | bool) -> Basic | None:
    """
...
def refine_Heaviside(expr: Basic, assumptions: Boolean | bool) -> Basic:
    """
...
def refine_floor_ceiling(expr: Basic, assumptions: Boolean | bool) -> Basic:
    """
...
handlers_dict: dict[str, Callable[[Basic, Boolean | bool], Basic | int | None]] = {
    'Abs': refine_abs,
    'Pow': refine_Pow,
    'atan2': refine_atan2,
    're': refine_re,
